...
DefaultMessage = f'''{Username} : Hello {Assistantname}, How are you today?
...
def ReadChatLogJson():
    with open(r'Data\ChatLog.json', 'r', encoding='utf-8') as file:
        content = file.read().strip()  # Read and strip spaces/newlines
        if not content:  # If file is empty, return empty list
            return []
        chatlog_data = json.loads(content)
        return chatlog_data[-10:]
       # return json.loads(content)  # Load JSON safely
